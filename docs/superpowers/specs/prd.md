...
| 功能模块 | 状态 | 功能描述 | 优先级 |
|---------|------|---------|--------|
| 统一递归分块 | ✅ | 按 `---` → 标题 → 段落 → 句子 递归分割 | P1 |
| 50% 重叠分块 | ✅ | 块与块之间保留 50% 重叠，确保上下文连续 | P1 |
| 分块配置 | ✅ | 可配置分块大小、重叠比例、最小块大小 | P1 |
| 向量多映射 | ✅ | 单个笔记生成多个向量分块，搜索时自动聚合 | P1 |
| 旧索引兼容 | ✅ | 支持重新分块并重建向量索引 | P1 |

**统一递归分块用户故事：**

- 作为用户，我想要笔记内容按语义层级递归分割（`---` → 标题 → 段落 → 句子），而不是固定长度截断，以便AI能准确检索到完整的信息
- 作为用户，我想要分块之间有 50% 重叠，以便AI在边界处能理解完整的上下文
- 作为用户，我想要配置分块策略参数（块大小、重叠比例），以便根据内容特点调整
- 作为用户，我想要对现有笔记重新分块并更新向量索引，以便享受新策略带来的准确度提升

**统一递归分块验收标准：**

| 验收项 | 验收标准 |
|--------|----------|
| 递归分割层级 | Level 1: `---` 分块标记 > Level 2: Markdown 标题 > Level 3: 段落 > Level 4: 句子 |
| 重叠机制 | 块与块之间保留 50% 重叠（400字符/块） |
| 块大小控制 | 单块最大 800 字符，最小 50 字符 |
| 语义完整 | 标题和其所属内容必须在同一块中 |
| 搜索聚合 | 多分块结果按笔记ID聚合，返回最高相似分块分数 |
| 向量数量 | 3000字符笔记预计产生4-6个向量（取决于结构） |
| 索引重建 | 支持一键重建所有笔记的向量索引 |

**当前问题 vs 优化后对比：**

| 维度 | 当前实现（固定长度截断） | 优化后（统一递归分块） |
|------|------------------------|------------------|
| 分块方式 | `content[:2048]` 固定截断 | `---` → 标题 → 段落 → 句子 递归分割 |
| 重叠机制 | ❌ 无 | ✅ 50% 重叠（400字符） |
| 信息保留 | ❌ 长笔记后半部分丢失 | ✅ 完整保留所有内容 |
| 语义完整性 | ❌ 段落可能被腰斩 | ✅ 按自然语义边界分割 |
| 向量数量 | 1个/笔记 | 多个/笔记（取决于长度） |
| 搜索精度 | 粗粒度，可能检索到截断片段 | 细粒度，检索语义完整的块 |
...
#### V3.7 功能（混合检索：向量 + BM25）✅ 新增

| 功能模块 | 状态 | 功能描述 | 优先级 |
|---------|------|---------|--------|
| BM25 关键词检索 | ✅ | 使用 BM25 算法进行关键词检索 | P1 |
| 向量语义检索 | ✅ | 保持现有的向量语义检索 | P1 |
| 混合检索融合 | ✅ | RRF (Reciprocal Rank Fusion) 算法融合两种检索结果 | P1 |
| 权重配置 | ✅ | 可配置向量检索和 BM25 的权重比例 | P1 |
| 召回率提升 | ✅ | 混合检索相比单一向量检索召回率提升 15-25% | P1 |

**混合检索用户故事：**

- 作为用户，我想要即使我的问题与笔记用词不同，也能被检索到，以便 AI 能回答语义相似但表述不同的问题
- 作为用户，我想要通过精确的关键词匹配找到包含特定术语的笔记，以便获取专业内容
- 作为用户，我想要向量检索和关键词检索的结果融合，以便获得更全面的检索结果

**混合检索验收标准：**

| 验收项 | 验收标准 |
|--------|----------|
| BM25 检索 | 支持中英文分词，返回相关度评分 |
| 混合融合 | 使用 RRF 算法融合，权重可配置 |
| 召回率 | 混合检索召回率比单一向量检索提升 15-25% |
| 响应时间 | 混合检索响应时间 < 1.5 秒 |
| 优雅降级 | BM25 不可用时自动回退到纯向量检索 |

**检索融合算法（RRF）：**

```
RRF Score = Σ (1 / (k + rank)) / (k + rank)

其中：
- rank: 该文档在不同检索结果中的排名
- k: 平滑参数（默认 60）
- 最终按 RRF Score 排序取 Top-K
```
...
#### V3.8 功能（RAG 效果评估体系）✅ 新增

| 功能模块 | 状态 | 功能描述 | 优先级 |
|---------|------|---------|--------|
| RAGAS 评估框架 | ✅ | 集成 RAGAS 框架实现自动化评估 | P1 |
| 答案相关性评估 | ✅ | 评估生成答案与问题的相关性 | P1 |
| 上下文相关性评估 | ✅ | 评估检索到的上下文与问题的相关性 | P1 |
| 忠实度评估 | ✅ | 评估生成答案对检索上下文的忠实度 | P1 |
| 答案准确率评估 | ✅ | 评估生成答案的事实准确性 | P1 |
| 评估数据集管理 | ✅ | 管理评估测试数据集，支持批量导入 | P1 |
| 评估报告生成 | ✅ | 自动生成评估报告，支持可视化展示 | P1 |

**RAG 评估用户故事：**

- 作为用户，我想要了解我的 RAG 系统效果如何，以便持续优化
- 作为用户，我想要在优化分块策略或提示词后量化效果提升，以便数据驱动决策
- 作为用户，我想要定期运行自动化评估，以便监控系统的长期表现

**RAGAS 评估指标：**

| 指标 | 说明 | 评分范围 |
|------|------|----------|
| Answer Relevance | 答案与问题的相关性 | 0-1 |
| Context Relevance | 检索上下文与问题的相关性 | 0-1 |
| Faithfulness | 生成答案对上下文的忠实度 | 0-1 |
| Answer Correctness | 答案的事实准确性 | 0-1 |
...
#### V3.9 功能（多模态支持：图片笔记）✅ 新增

| 功能模块 | 状态 | 功能描述 | 优先级 |
|---------|------|---------|--------|
| CLIP 模型集成 | ✅ | 使用 CLIP 模型实现图片向量化 | P1 |
| 图片笔记创建 | ✅ | 支持创建包含图片的笔记 | P1 |
| 图片语义搜索 | ✅ | 通过自然语言搜索图片笔记 | P1 |
| 图片问答 | ✅ | 支持对图片内容进行问答 | P1 |
| 跨模态检索 | ✅ | 同时支持文本和图片的混合检索 | P2 |
| 图片缩略图 | ✅ | 自动生成图片缩略图用于预览 | P1 |

**多模态用户故事：**

- 作为用户，我想要用自然语言搜索我的图片，例如「找到包含猫的图片」，以便快速找到图片笔记
- 作为用户，我想要对图片内容提问，例如「这张图里的建筑是什么风格的」，以便理解图片内容
- 作为用户，我想要创建图文结合的笔记，以便更好地记录和整理知识

**CLIP 模型原理：**

```
文本编码：Text → CLIP Text Encoder → 文本向量（512/768维）
图片编码：Image → CLIP Image Encoder → 图片向量（512/768维）

文本-图片相似度 = cos(文本向量, 图片向量)
```
...
| 组件 | 选项 | 说明 |
|------|------|------|
| CLIP 模型 | `openai/clip-vit-base-patch32` | 开源 CLIP 模型，支持中英文 |
| 部署方式 | 本地推理 | 使用 HuggingFace Transformers |
| 向量维度 | 512 | CLIP 默认维度 |
| 图片预处理 | PIL + torchvision | 统一图片尺寸和格式 |
...
| 功能模块 | 状态 | 功能描述 | 优先级 |
|---------|------|---------|--------|
| PDF 导入 | ✅ | 支持上传 PDF 文件，自动解析文本并导入知识库 | P1 |
| 网页导入 | ✅ | 支持输入 URL，自动抓取网页正文并导入知识库 | P1 |
| 视频导入 | ✅ | 支持 B站/YouTube 视频链接，自动下载并提取字幕导入 | P1 |
| AI 摘要生成 | ✅ | 使用 LLM 自动生成导入内容的摘要和关键词 | P1 |
| 智能分类 | ✅ | 根据内容自动推荐分类和标签 | P1 |
| 导入进度 SSE | ✅ | 通过 SSE 实时推送导入任务进度 | P1 |
| 任务管理 | ✅ | 支持导入任务查看、重试、删除等操作 | P1 |
| 导入历史 | ✅ | 记录所有导入历史，支持筛选和查看 | P1 |

**智能导入用户故事：**

- 作为用户，我想要上传 PDF 文档自动导入到知识库，以便快速整理资料
- 作为用户，我想要输入网页 URL 自动抓取正文并导入，以便保存有价值的网络文章
- 作为用户，我想要输入视频链接自动提取字幕并导入，以便将视频内容转化为可检索的文本
- 作为用户，我想要看到导入任务的实时进度，以便了解处理状态
- 作为用户，我想要系统自动为导入内容生成摘要和标签，以便快速整理
...
#### V3.11 功能（非Markdown文件预处理Pipeline）✅ 新增

> **核心目标**：让 RAG 系统支持 PDF、图片、网页等非 Markdown 文件的精准检索。

**功能架构（4阶段Pipeline）：**

| 阶段 | 功能 | 状态 | 说明 |
|------|------|------|------|
| **第一阶段：文档解析** | 按类型解析 | ✅ | PDF/网页/图片/PPT/Word |
| | 结构保留 | ✅ | 标题、段落、表格、页码 |
| **第二阶段：内容清洗** | 噪声去除 | ✅ | 页眉页脚、广告、导航栏 |
| | 乱码清理 | ✅ | 去除无效字符、冗余空白 |
| **第三阶段：语义分块** | 递归分块 | ✅ | 按语义边界切分 |
| | 重叠机制 | ✅ | 块间保留 20-30% 重叠 |
| | 元数据注入 | ✅ | 来源、页码、时间等 |
| **第四阶段：向量化入库** | Embedding | ✅ | 通义千问 text-embedding-v3 |
| | FAISS 索引 | ✅ | 构建向量索引支持检索 |
...
| 文件类型 | 解析技术 | 说明 |
|----------|----------|------|
| 文本型 PDF | PyMuPDF | 提取正文 + 结构（标题、段落、表格） |
| 扫描版 PDF | OCR (Tesseract/PaddleOCR) | 图片转文字 |
| 网页 | Trafilatura | 自动去除广告/导航栏，只留正文 |
| 图片 | OCR (EasyOCR/Tesseract) | 提取图片文字内容 |
| PPT | python-pptx | 提取每页标题 + 正文 |
...
|----------|----------|
| 去除页眉页脚 | 连续多页出现的相同短文本 |
| 去除广告噪声 | 扫码关注、微信公众号、版权声明等 |
| 去除乱码 | \x00-\x08、特殊空白字符 \u200b |
| 冗余空白 | 多个空行合并、去除行首行尾空白 |

**语义分块方案：**

| 分块策略 | 说明 |
|----------|------|
| 分块层级 | Level 1: --- 分隔 > Level 2: Markdown标题 > Level 3: 段落 > Level 4: 句子 |
| 重叠机制 | 相邻块保留 20-30% 重叠，防止上下文断裂 |
| 块大小控制 | 最大 800 字符，最小 50 字符 |

**元数据注入方案：**

| 元数据类型 | 说明 |
|------------|------|
| 来源信息 | source（文件名/URL）、source_type（pdf/web/image） |
| 位置信息 | page_num（页码）、chunk_index（块索引） |
| 时间信息 | crawl_time（抓取时间）、publish_time（发布时间） |
| 统计信息 | word_count（字数）、heading（所属标题） |
...
│ 第一阶段：文档解析                                             │
├─────────────┬─────────────┬─────────────┬───────────────────┤
│ PDF解析     │ 图片OCR     │ 网页抽取    │ Office解析        │
│ PyMuPDF     │ Tesseract  │ Trafilatura │ python-docx      │
└─────────────┴─────────────┴─────────────┴───────────────────┘
      │
      ▼
┌─────────────────────────────────────────────────────────────┐
...
- 作为用户，我想要上传扫描版 PDF 并自动 OCR 识别为文字，以便将纸质资料纳入 RAG 检索
- 作为用户，我想要导入包含图片的网页并提取图片文字，以便图片内容也能被检索到
- 作为用户，我想要导入的文档自动去除页眉页脚和广告，只保留核心正文，以便提高检索精度
- 作为用户，我想要分块不是生硬截断，而是按段落边界切分，以便 AI 能理解完整语义
- 作为用户，我想要 AI 回答时能标注答案来自哪个文档的第几页，以便我回溯原文

**非Markdown文件预处理验收标准：**

| 验收项 | 验收标准 |
|--------|----------|
| 文本型 PDF 解析 | 正确提取正文，保留标题层级和段落结构 |
| 扫描版 PDF 解析 | OCR 识别率 > 95%（清晰图片） |
| 网页正文提取 | 去除广告、导航栏、侧边栏，只留正文 |
| 内容清洗 | 无乱码、无冗余空白、无页眉页脚 |
| 语义分块 | 按段落/标题边界切分，不是固定长度截断 |
| 块间重叠 | 保留 20-30% 重叠，上下文连续 |
| 元数据完整性 | 每个 chunk 携带 source、page_num、crawl_time |
| 向量化入库 | 所有 chunk 正确生成向量并入库 |
| 答案溯源 | AI 回答时能标注来源文档和页码 |
...
| 数据导入导出 | Markdown/JSON 格式导入导出 | P1 | ✅ 已完成 |
| 网页内容提取 | 自动提取网页正文 | P1 | ✅ 已完成 |
| 视频字幕分析 | 提取视频字幕进行内容分析 | P1 | ✅ 已完成 |
...
|---------|---------|--------|------|
| 语义搜索 | 基于向量的语义搜索 | P1 | ✅ 已完成 |
| 数据导入导出 | Markdown/JSON 格式导入导出 | P1 | ✅ 已完成（智能导入）|
| 网页内容提取 | 自动提取网页正文 | P1 | ✅ 已完成（智能导入）|
| 视频字幕分析 | 提取视频字幕进行内容分析 | P1 | ✅ 已完成（智能导入）|
| 浏览器插件 | 一键保存网页内容 | P2 | ⏳ 待实现 |
| 批量导入 | 批量导入现有知识库 | P1 | ⏳ 待实现 |
...
| v2.0.1 | 2026-03-22 | 产品团队 | 统一递归分块：按 `---` → 标题 → 段落 → 句子 递归分割，50%重叠分块 |
| v2.0.2 | 2026-03-22 | 产品团队 | 混合检索：向量 + BM25 融合，RRF 算法，召回率提升 15-25% |
| v2.0.3 | 2026-03-23 | 产品团队 | RAG 评估体系：RAGAS 框架集成，四维自动化评估 |
| v2.0.4 | 2026-03-24 | 产品团队 | 多模态支持：CLIP 图片检索问答，跨模态检索 |
| v2.1.0 | 2026-04-22 | 产品团队 | 智能导入模块：PDF/网页/视频导入，AI摘要生成，智能分类，任务管理 |
